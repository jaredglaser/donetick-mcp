...
14. **One predicate, and one sentence.** When a write refuses a shape, the read that describes it comes from the same function; when two modules tell the caller the same thing, one of them calls the other. Two copies drift, and each reads as plausible on its own.
...
17. **A wire value that decides a branch gets a name.** Paths live in `src/endpoints.ts` and nothing calls a literal one; a status, a bound or a sentinel compared inline is the same defect a level down. Derive the name from the table that already holds it.
...
- `src/tools/read.ts`, `write.ts`, `schedule.ts`, `actions.ts` and `subtasks.ts` implement the twenty tools; `src/tools/chore-lookup.ts` is the single loader they all resolve a chore id through, `get_chore` and `delete_chore` included, and it owns both the diagnosis that separates a chore that is genuinely absent from one this server cannot describe and from a read that simply failed, and the retry every whole-chore write shares when Donetick refuses a stale version; `src/tools/context.ts` holds the `ToolContext` every handler takes, in its own module so that no tool module owns it and none has to import a peer to get it; `src/tools/index.ts` declares them as plain data, each with the MCP annotations a client reads to tell a reader from a destructive write.
